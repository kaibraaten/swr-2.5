...
DO_FUN *skill_function( const char *name )
{
  switch ( name[3] )
  {
	...
	return do_consider;
      if( !str_cmp( name, "do_construction" ) )
	return do_construction;
	...
	return do_description;
      if( !str_cmp( name, "do_designship" ) )
	return do_designship;
	...
	return do_mp_deposit;
      if( !str_cmp( name, "do_mp_offer_job" ) )
	return do_mp_offer_job;
	...
	return do_setplanet;
      if( !str_cmp( name, "do_setprototype" ) )
	return do_setprototype;
      if( !str_cmp( name, "do_setwages" ) )
	return do_setwages;
	...
	return do_showplanet;
      if( !str_cmp( name, "do_showprototype" ) )
	return do_showprototype;
...
const char *skill_name( DO_FUN * skill )
{
  ...
  if( skill == do_war )
    return "do_war";
  if( skill == do_setwages )
    return "do_setwages";
  if( skill == do_prototypes )
    return "do_prototypes";
  if( skill == do_setprototype )
    return "do_setprototype";
  if( skill == do_showprototype )
    return "do_showprototype";
  if( skill == do_rentship )
    return "do_rentship";
  if( skill == do_designship )
    return "do_designship";
  if( skill == do_disarm )
    return "do_disarm";
  if( skill == do_resign )
  ...
  if( skill == do_survey )
    return "do_survey";
  if( skill == do_landscape )
    return "do_landscape";
  if( skill == do_construction )
    return "do_construction";
  if( skill == do_allsave )
    return "do_allsave";
  if( skill == do_teach )
    return "do_teach";
  if( skill == do_quicktalk )
    return "do_quicktalk";
  if( skill == do_bridge )
    return "do_bridge";
