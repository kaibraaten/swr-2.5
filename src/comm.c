...
SOCKET init_socket(int listen_port)
{
    ...
    if((setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &optval, optlen)) == SOCKET_ERROR)
    {
...
#if defined(SO_DONTLINGER) && !defined(SYSV) && !defined(__sun__)
    {
        ...
        if(setsockopt(fd, SOL_SOCKET, SO_DONTLINGER, (void *) &ld, sizeof(ld)) == SOCKET_ERROR)
        {
...
void close_socket(DESCRIPTOR_DATA *dclose, bool force)
{
    ...
    {
        DESCRIPTOR_DATA *dp = NULL, *dn = NULL;
        bug("Close_socket: %s desc:%p != last_desc:%p and desc->next = NULL!", ch ? ch->name : d->host, dclose,
            last_descriptor);
...
#else
        ssize_t nRead = recv(d->descriptor, bufin, sizeof(bufin) - 10 - iStart, 0);
#endif
        ...
        case '%':
            *pbuf = '\0';
            ...
            case 'i':
                ...
                break;
            case 'I':
                p_stat = (IS_NPC(ch) ? (IS_SET(ch->act, ACT_MOBINVIS) ? ch->mobinvis : 0) : (IS_SET(ch->act,
                                                                                                    PLR_WIZINVIS)
                                                                                             ? ch->pcdata->wizinvis
                                                                                             : 0));
                break;
...
bool pager_output(DESCRIPTOR_DATA *d)
{
    ...
    if(IS_SET(ch->act, PLR_ANSI))
    {
        ...
        {
            sprintf(buf, "\033[0;%d;%s%dm", (d->pagecolor & 8) == 8, (d->pagecolor > 15 ? "5;" : ""),
                    (d->pagecolor & 7) + 30);
        }
