...
void violence_update(void)
{
    ...
    for(ch = last_char; ch; lst_ch = ch, ch = gch_prev)
    {
        ...
        if(!ch->in_room || !ch->name)
        {
            ...
            sprintf(buf, "ch: %ld  ch->in_room: %ld  ch->prev: %ld  ch->next: %ld", (long) ch, (long) ch->in_room,
                    (long) ch->prev, (long) ch->next);
            log_string(buf);
            log_string(lastplayercmd);
            if(lst_ch)
            {
                sprintf(buf, "lst_ch: %ld  lst_ch->prev: %ld  lst_ch->next: %ld", (long) lst_ch, (long) lst_ch->prev,
                        (long) lst_ch->next);
            }
            else
            {
        ...
        for(rch = ch->in_room->first_person; rch; rch = rch_next)
        {
            ...
            if(IS_AWAKE(rch) && !rch->fighting)
            {
                ...
                if(IS_NPC(rch) && !IS_AFFECTED(rch, AFF_CHARM) && !IS_SET(rch->act, ACT_NOASSIST))
                {
                    ...
                    if(rch->pIndexData == ch->pIndexData || (IS_NPC(ch) && IS_SET(ch->act, ACT_CITIZEN)))
                    {
                        CHAR_DATA *vch;
                        CHAR_DATA *target;
                        int number;
...
ch_ret multi_hit(CHAR_DATA *ch, CHAR_DATA *victim, int dt)
{
    ...
    */
    if(retcode == rNONE)
    {
        ...
        {
            move = encumbrance(ch, movement_loss[UMIN
            (SECT_MAX - 1, ch->in_room->sector_type)]);
        }
        else
        {
...
ch_ret damage(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt)
{
    ...
    if(victim != ch)
    {
        ...
        if(dt >= TYPE_HIT)
        {
            if(IS_NPC(ch) && IS_SET(ch->attacks, DFND_DISARM) && number_percent() < ch->top_level / 2)
            {
                disarm(ch, victim);
            }

            if(IS_NPC(ch) && IS_SET(ch->attacks, ATCK_TRIP) && number_percent() < ch->top_level)
            {
                trip(ch, victim);
            }
    ...
    if(dam > 0 && dt > TYPE_HIT && !IS_AFFECTED(victim, AFF_POISON) && is_wielding_poisoned(ch)
       && !IS_SET(victim->immune, RIS_POISON) && !saves_poison_death(ch->top_level, victim))
    {
        AFFECT_DATA af;
    ...
    if(victim->position == POS_DEAD)
    {
        ...
        if(!npcvict)
        {
            sprintf(log_buf, "%s killed by %s at %ld", victim->name, (IS_NPC(ch) ? ch->short_descr : ch->name),
                    victim->in_room->vnum);
            log_string(log_buf);
...
void stop_fighting(CHAR_DATA *ch, bool fBoth)
{
    ...
    if(!fBoth)
    {                        /* major short cut here by Thoric */
        return;
    }
...
void raw_kill(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    if(victim->in_room && IS_NPC(victim) && victim->in_room->area && victim->in_room->area->planet)
    {
        ...
        else if(IS_SET(victim->act, ACT_CITIZEN))
        {
            ...
            victim->in_room->area->planet->pop_support -= (float) (1 + 1 / (victim->in_room->area->planet->population
                                                                            + 1));
    ...
    sprintf(buf, "%s%c/%s", PLAYER_DIR, tolower((int) arg[0]), capitalize(arg));
    sprintf(buf2, "%s%c/%s", BACKUP_DIR, tolower((int) arg[0]), capitalize(arg));

    rename(buf, buf2);

    sprintf(buf, "%s%c/%s.clone", PLAYER_DIR, tolower((int) arg[0]), capitalize(arg));
    sprintf(buf2, "%s%c/%s", PLAYER_DIR, tolower((int) arg[0]), capitalize(arg));
...
void dam_message(CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt)
{
    ...
    else
    {
        ...
        else if(dt >= TYPE_HIT && dt < (int) (TYPE_HIT + attack_table_size()))
        {
            attack = attack_table[dt - TYPE_HIT];
        }
        else
