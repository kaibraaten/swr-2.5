...
static void ship_crash_with_star( SHIP_DATA *ship, const char *star_name )
{
  char buf[MAX_STRING_LENGTH];
...
void move_ships()
{
  ...
  for ( ship = first_ship; ship; ship = nextship )
	...
	continue;
	  ...
	  && vector_distance( &ship->pos, &ship->starsystem->star1_pos ) < 10 )
	{
	  ...
	  && vector_distance( &ship->pos, &ship->starsystem->star2_pos ) < 10 )
	{
	  ...
	  PLANET_DATA *planet = NULL;
	    ...
	    {
	      if( ship_distance_to_planet( ship, planet ) < 10 )
		{
...
static void ship_handle_autofly_combat( SHIP_DATA *ship )
{
  int chance = 100;
...
static void ship_reload_missiles( SHIP_DATA *ship )
{
...
static void ship_update_laserstate( SHIP_DATA *ship )
{
...
static void ship_update_shipstate( SHIP_DATA *ship )
{
...
static void ship_update_hyperspace( SHIP_DATA *ship )
{
...
static void ship_update_shields( SHIP_DATA *ship )
{
  ...
  if (ship->autorecharge
      ...
      && ship->energy > 100)
    {
      int recharge = UMIN( ship->maxshield-ship->shield, 10 + ship->model*2 );
...
static void ship_show_space_prompt( const SHIP_DATA *ship )
{
...
static void ship_echo_proximity_alert( const SHIP_DATA *ship,
...
static void ship_check_proximity( const SHIP_DATA *ship )
{
  if ( ship->starsystem )
    {
...
static void ship_show_combat_prompt( SHIP_DATA *ship )
{
  if (ship->target)
    {
      char buf[MAX_STRING_LENGTH];
...
static void ship_warn_on_low_fuel( const SHIP_DATA *ship )
{
...
static void ship_update_autotrack( SHIP_DATA *ship )
{
...
static void spacestation_regenerate_missiles( SHIP_DATA *ship )
{
  if ( ship->ship_class == SPACE_STATION
       && ship->target == NULL )
    {
...
{   
    ...
    ch_printf( ch, "%s %s : %s\r\nFilename: %s\r\n",
		       ...
		       (ship->ship_class == LAND_VEHICLE ? "land vehicle" : "Unknown" ) ) ) ) ), 
    			ship->name,
    			ship->filename);
...
void do_ships( CHAR_DATA *ch, char *argument )
{
  ...
  for ( ship = first_ship; ship; ship = ship->next )
	...
	{
	  if ( !ch->pcdata || !ch->pcdata->clan || str_cmp(ship->owner,ch->pcdata->clan->name) || ship->ship_class > SPACE_STATION )
	    continue;
	}
...
void do_speeders( CHAR_DATA *ch, char *argument )
{
  ...
  for ( ship = first_ship; ship; ship = ship->next )
      ...
      if ( str_cmp(ship->owner, ch->name) )
	{
      ...
      if ( ship->location != ch->in_room->vnum || ship->ship_class <= SPACE_STATION)
	continue;
      ...
      if  ( ship->in_room )       
	{
	  if ( ship->in_room->area && ship->in_room->area && ship->in_room->area->planet )
	    ch_printf( ch, "%s  -%s (%s)\r\n", ship->name, ship->in_room->name, ship->in_room->area->planet->name );
	  else
	    ch_printf( ch, "%s  -%s\r\n", ship->name, ship->in_room->name );
	}
      else 
...
void do_info(CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch, "%s%s : %s\r\n",
		       ...
		       (target->ship_class == LAND_VEHICLE ? "land vehicle" : "Unknown" ) ) ) ) ), 
    			target->name);
...
void do_hyperspace(CHAR_DATA *ch, char *argument )
{
    ...
    char buf[MAX_STRING_LENGTH];
	   ...
	   if( ship_distance_to_ship( eShip, ship ) < 500 )
           {
