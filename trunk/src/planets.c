...
static void make_colony_center( PLANET_DATA * planet,
    ROOM_INDEX_DATA * location )
{
  char buf[MAX_STRING_LENGTH];
  *buf = '\0';
...
static void make_colony_shuttle_platform( PLANET_DATA * planet,
    ROOM_INDEX_DATA * location )
{
  char buf[MAX_STRING_LENGTH];
  *buf = '\0';
...
static void make_colony_hotel( PLANET_DATA * planet,
    ROOM_INDEX_DATA * location )
{
  char buf[MAX_STRING_LENGTH];
  *buf = '\0';
...
static void make_colony_wilderness( PLANET_DATA * planet,
    ROOM_INDEX_DATA * location,
...
void do_makeplanet( CHAR_DATA * ch, char *argument )
{
  ...
  switch ( ch->substate )
  {
	...
	if( rnum > 5
	    && rnum != 23
	    && rnum != 17
	    && rnum != 19 && rnum != COLONY_ROOM_SUPPLY_SHOP && rnum != 14 )
	{
	...
	if( rnum != 1
	    ...
	    && rnum != 15 && rnum != COLONY_ROOM_HOTEL && rnum != 19 )
	{
